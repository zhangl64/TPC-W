...
public class EB implements Runnable {
  ...
  public void initialize() {
	...
	//log("initialize() - lei");
	//lei_09
	//log("usmd = " + (usmd-System.currentTimeMillis()) + " - lei");
  }
  ...
  public void run() {
    ...
    //lei_open
    //System.out.println("lei: EB " + name + " - Thread is running.");
	...
	if (terminate) { 
		//lei_open
		System.out.println("lei: committing suicide at the beginning.");
	...
	//if ((wirt_t1 >= usmd) && (toHome)) { //lei_10 
	if (toHome) { //lei_10 
	    // Start new user session.
	    //log("Completed user session");
		
		//lei_09 start***
		//instead of "return", if queueCounter>0, we need to "initialize" and "continue".
		RBE.userCounter--;
		System.out.println("lei: "+name+" completed, userCounter = " + RBE.userCounter);
		//handling negative userCounter 
		if(RBE.userCounter < 0)
			{
			System.out.println("lei: "+name+" - error, userCounter < 0.");
			RBE.userCounter = 0;
			}

		if(RBE.queueCounter > 0)
			{
			initialize();
			RBE.queueCounter--;
			System.out.println("lei: "+name+" - restarted, with queueCounter = "+RBE.queueCounter);
			//handling negative queueCounter
			if(RBE.queueCounter< 0)
				{
				System.out.println("lei: "+name+" - error, queueCounter < 0.");
				RBE.queueCounter= 0;
				}
			RBE.userCounter++;
			System.out.println("lei: "+name+" - userCounter back to " + RBE.userCounter);
			continue;
			}
		else if(RBE.queueCounter == 0)
			{
			//System.out.println("lei: "+name+" - queueCounter is empty."); //just for debug
			//lei_10 test purpose: 
			//initialize();
			//continue;
			return;
			...
			{
			System.out.println("lei: "+name+" - error, queueCounter.");
			return;
			}
		//lei_09 end***
	...
	if (terminate) {
		//lei_open
		System.out.println("lei: committing suicide at the end.");
	    if (DEBUG > 0) {
		log("Committing suicide");
	    }
	    return;
...
// lei_open, nextState() first called by initialize(), then by run().
  void nextState(String html)
  {
	...
	toHome = trans[curState][j].toHome();	//lei_10 if the curState is Home, it is true; otherwise, false.
	//lei_06
	//log("lei: toHome = " + toHome + ", From " + curState + " To " + j + " Via: " + nextReq);
	if (DEBUG>2) {
