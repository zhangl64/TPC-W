...
public class EBWAdminConfTrans extends EBTransition {
  ...
  public String request(EB eb, String html) {
    ...
    RBE rbe = eb.rbe;
	 ...
	 i=iid.find(html);
	 if (i==-1) {
		rbe.stats.error("Unable to find I_ID in admin confirm page.", "???", null); //lei add null
		return("");
	 }
	 i=i+iid.length();
	 ...
	 if (e==-1) {
		rbe.stats.error("Unable to find end of I_ID in admin confirm page.", "???", null); //lei add null
		return("");
	 }
	 e=e+i;
	 ...
	 url = RBE.adminConfURL + "?" + RBE.field_newimage + "=" + 
		...
		Pad.lz(2,RBE.unifCents(eb.rand)) + "&" +
	     RBE.field_newstock + "=" + RBE.unifStock(eb.rand) + "&" +
		RBE.field_iid + "=" + id;
