...
public class EBWAdminReqTrans extends EBTransition {
  ...
  public String request(EB eb, String html) {
    ...
    RBE rbe = eb.rbe;
	 ...
	 i=iid.find(html);
	 if (i==-1) {
		rbe.stats.error("Unable to find I_ID in admin request page.", "???", null); //lei add null
		return("");
	 }
	 i=i+iid.length();
	 ...
	 if (e==-1) {
		rbe.stats.error("Unable to find end of I_ID in admin request page.", "???", null); //lei add null
		return("");
	 }
	 e=e+i;
