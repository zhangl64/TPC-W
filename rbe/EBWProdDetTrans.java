  ...
  public String request(EB eb, String html) {
    ...
    RBE rbe = eb.rbe;
	 ...
	 for (i = itemPat.find(html);i!=-1;i=itemPat.find(html, i)) {
		...
		iid.addElement(Integer.valueOf(html.substring(s,e)));
	 }

	 if (iid.size()==0) {
		rbe.stats.error("Unable to find any items for product detail.", "???", null); //lei add null
		return("");
	 }

	 i = eb.nextInt(iid.size());
	 i = iid.elementAt(i).intValue();
