...
public class EBWShopCartRefTrans extends EBWShopCartTrans {
  ...
  private static final StrStrPattern iidPat = new StrStrPattern("NAME=\"I_ID");
  private static final StrStrPattern iidValuePat = new StrStrPattern("value = \"");
  ...
  public String request(EB eb, String html) {
	 ...
	 /* Get the item IDs. */
	 int [] iid = new int[c];
	 for (c=0,i=iidPat.find(html,0);i!=-1;i=iidPat.find(html,i+1),c++) {
		int j = iidValuePat.find(html, i+iidPat.length()) + iidValuePat.length();
		int e = CharSetStrPattern.notDigit.find(html,j);
		iid[c] = Integer.parseInt(html.substring(j,e));
	 }
	 ...
	 url = RBE.shopCartURL + "?" + RBE.field_addflag + "=UPDATE";

	 for (i=0;i<c;i++) {
		url = url + "&" + RBE.field_iid + "_" + i + "=" + iid[i] + "&" + RBE.field_qty + "_" + i + "=" + qtyNew[i];
	 }
